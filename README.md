...
Методы класса:
- `setCatalog` - Устанавливает список доступных товаров.
- `setPreview` - Устанавливает идентификатор товара для предварительного просмотра
- `addToBasket` - Добавляет товар в корзину
- `removeFromBasket` - Удаляет товар из корзины по идентификатору.
- `setOrder` - Устанавливает состояние заказа.
- `clearOrder` - Очищает состояние заказа
- `calculateTotal` - Вычисляет общую сумму заказа на основе товаров в корзине.
- `createOrderForServer` - Создает объект заказа для передачи на сервер, вычисляя итоговую сумму и составляя массив идентификаторов товаров
## Слой представления
...
Поля:
- `_counter:HTMLElement` кол-во товаров в корзине
...
- `basketButton:HTMLElement` - кнопка которая будет открывать корзину 

методы:
- `set counter` - устанавливает значение кол-ва товаров в корзине
...
- `button: HTMLElement` - Кнопка для оформления заказа(дальнейший переход IPaymentForm)
...
Класс карточки товара и отображения ее данных. Универсальный класс в котором можно указать к какому блоку нужна разметка карточки чтобы она работала для всех вариантов отображения.
конструктор класса:

constructor(blockName: string, container: HTMLElement,events: IEvents) принимает имя блока, элемент DOM и действие над карточкой
...
- `cardButton: HTMLElement` - слушатель кнопки карточки

### Класс SuccessfulPurchase
Класс предназначенный для отрожения модального окна успешной покупки.
...
- `card:changed` - изменение данных о карточке которая отображается в модальном окне в подробном варианте
